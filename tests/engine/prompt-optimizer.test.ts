...
describe('prompt-optimizer', () => {
  ...
  it('更新指标', () => {
    ...
    expect(updateVersionMetrics('1.0', { totalCalls: 100, averageAccuracy: 0.75, byClaimType: {} as unknown, byCategory: {} as unknown })).toBe(true)
    expect(getActivePromptVersion()?.metrics?.totalCalls).toBe(100)
  })
  it('版本对比', () => {
    registerPromptVersion(v1); registerPromptVersion(v2)
    updateVersionMetrics('1.0', { totalCalls: 50, averageAccuracy: 0.6, byClaimType: {} as unknown, byCategory: {} as unknown })
    updateVersionMetrics('1.1', { totalCalls: 50, averageAccuracy: 0.8, byClaimType: {} as unknown, byCategory: {} as unknown })
