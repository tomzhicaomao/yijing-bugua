  ...
  if (records.length === 0) {
    return (
      <div className="max-w-lg mx-auto py-6 space-y-6">
        ...
        <p className="text-center py-8 text-gray-400">暂无数据，开始你的第一次占卜吧</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-lg mx-auto py-6 space-y-6">
      ...
      <div className="border rounded-lg p-4">
        <h3 className="font-medium mb-3">按分类统计</h3>
        <div className="space-y-2">
          {byCategory.map(c => (
            <div key={c.category} className="flex items-center justify-between text-sm">
              <span className="font-medium">{c.category}</span>
              <span className="text-gray-500">{c.total}条</span>
              <span>{c.base >= 5 ? "准确率 " + Math.round(c.a / c.base * 100) + "% (" + c.base + "条)" : "样本不足 (" + c.base + "条)"}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {allInterpretations.length > 0 && (
        <div className="border rounded-lg p-4 text-sm space-y-2">
          <h3 className="font-medium">AI 解读统计</h3>
          <p>默认解读: {defaultInts.length} 次 | 深度分析: {deepInts.length} 次</p>
          {multiVersion && <p className="text-yellow-600">样本包含 {promptVersions.length} 个 Prompt 版本（{promptVersions.join(", ")}），比较结果仅供参考</p>}
          {allInterpretations.length >= 20 ? (
            <p>置信度校准数据充足（{allInterpretations.length} 条），可进行模式分析</p>
          ) : allInterpretations.length > 0 ? (
            <p className="text-gray-400">再积累一些反馈数据后，这里会显示分析模式（当前 {allInterpretations.length} 条，需 ≥20）</p>
          ) : null}
          {records.some(r => r.duplicate) && <p className="text-orange-600">存在重复占问记录，可能影响准确率统计</p>}
        </div>
      )}
    </div>
  )
...
function StatCard({ label, value, warn }: { label: string; value: string; warn?: boolean }) {
  return (
    <div className="border rounded-lg p-3 text-center">
      <p className="text-2xl font-bold" style={{color: warn ? "#9ca3af" : undefined}}>{value}</p>
